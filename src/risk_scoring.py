...
def compute_risk_scoring(df):
    ...
    for idx, row in out_df.iterrows():
        ...
        if exp_amt > sanc_amt:
            ...
            base_score += pts
            factors.append(f"Budget Overrun (+{round(pts,1)} pts): Actual expenditure exceeds sanctioned budget by {round(overrun_pct,1)}%")
        ...
        if util_pct > 80.0 and progress_pct < 50.0:
            base_score += 25.0
            factors.append(f"High Spending with Low Progress (+25 pts): {round(util_pct,1)}% funds spent while physical work is only {round(progress_pct,1)}% complete")
        elif util_pct < 20.0 and row.get('WORK_STATUS') == 'Delayed':
            base_score += 20.0
            factors.append(f"Delayed Project Start (+20 pts): Project schedule is delayed with only {round(util_pct,1)}% fund utilization")
        ...
        if row.get('IS_ANOMALY', False):
            ...
            base_score += anom_pts
            factors.append(f"Unusual Spending Pattern (+{round(anom_pts,1)} pts): Financial allocation and expenditure timeline differ significantly from standard projects in this category")

        # 4. Duplicate Similarity Risk (+15 pts)
        if row.get('IS_DUPLICATE_FLAG', False):
            base_score += 15.0
            factors.append("Possible Duplicate Project (+15 pts): Work title and budget closely match another project approved in the same district")
        ...
        if row.get('IS_CROSS_SCHEME_DUPLICATE', False):
            ...
            base_score += 30.0
            factors.append(f"Double-Funding Alert (+30 pts): A parallel project under {cs_name} is registered within {dist_m}m of this site")

        # 6. Cartel & Split-Tendering (+25 pts)
        if row.get('IS_SPLIT_TENDER', False):
            base_score += 25.0
            factors.append("Potential Tender Splitting (+25 pts): Multiple small contracts under ₹25 Lakh awarded in close succession, which may bypass competitive bidding")

        # 7. S-Curve Progress Stagnation (+20 pts)
        if row.get('IS_STAGNANT_SCURVE', False):
            base_score += 20.0
            factors.append("Funds Disbursed but Work Stalled (+20 pts): Financial releases have advanced significantly faster than physical progress on the ground")
